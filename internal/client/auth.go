...
// time the internal sends a new secret, the current secret rotates its secret
// to be the prior secret.  The client sends the prior secret with every launch,
// because this allows the internal to know when the client has gone out of sync
// (for example, when a client moves from apns dev to apns prod),
// and the internal rotates the secret when that happens.  Clients sign auth requests
// with the current secret, but the internal allows use of the prior
